...
class TestCode42Connector(object):
    def test_handle_test_connectivity(self, mocker, mock_py42_client, connector):
        param = {}
        mocker.patch("phantom.base_connector.BaseConnector.add_action_result", return_value=ActionResult(dict(param)))
        mocker.patch("py42.sdk.from_local_account", return_value=mock_py42_client)
    ...
    def test_handle_test_connectivity_calls_users_get_current(self, mocker, mock_py42_client, connector):
        param = {}
        mocker.patch("py42.sdk.from_local_account", return_value=mock_py42_client)
        mocker.patch("phantom.base_connector.BaseConnector.add_action_result", return_value=ActionResult(dict(param)))
    ...
    def test_handle_connectivity_sets_error_status_if_sdk_throws_exception(self, mocker, mock_py42_client, connector):
        param = {}
        mocker.patch("py42.sdk.from_local_account", return_value=mock_py42_client)
        mocker.patch("phantom.base_connector.BaseConnector.add_action_result", return_value=ActionResult(dict(param)))
    ...
    def test_handle_action_calls_handle_connectivity(self, mocker, mock_py42_client, test_connectivity_connector):
